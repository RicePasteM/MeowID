...
class DETRPoseHungarianMatcher(nn.Module):
    def __init__(self, cost_class: float = 1, focal_alpha=0.25,
        ...
        if keypoint_oks_sigmas is not None:
            if len(keypoint_oks_sigmas) != num_body_points:
                raise ValueError(
                    f'Expected {num_body_points} keypoint sigmas, '
                    f'got {len(keypoint_oks_sigmas)}'
                )
            self.sigmas = np.asarray(keypoint_oks_sigmas, dtype=np.float32)
        ...
        elif num_body_points==9:
            self.sigmas = np.array([
                .30, .30, .35, .50, .50, .50, .50, .50, .50
            ], dtype=np.float32) / 10.0
        else:
    ...
    def forward(self, outputs, targets):
        ...
        if Z_pred.sum() > 0:
            ...
            C = torch.nan_to_num(C, nan=0.0, posinf=1e8, neginf=-1e8)

        else:
            cost_keypoints = cost_oks = torch.tensor(0.0)
            C = self.cost_class * cost_class + self.cost_keypoints * cost_keypoints + self.cost_oks * cost_oks
            C = C.view(bs, num_queries, -1).cpu()
            C = torch.nan_to_num(C, nan=0.0, posinf=1e8, neginf=-1e8)
        ...
        for i, c in enumerate(C.split(sizes, -1)):
            ...
            if not np.isfinite(cost_mat).all():
                cost_mat = np.nan_to_num(cost_mat, nan=0.0, posinf=1e8, neginf=-1e8)
            indices.append(linear_sum_assignment(cost_mat))
