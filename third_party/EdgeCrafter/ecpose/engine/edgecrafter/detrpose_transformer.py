...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def with_pos_embed(tensor, pos, training):
        if pos is not None:
            np = pos.shape[2]
            # Keep this operation out-of-place. ``tensor`` is reused by the
            # class head through a view of its instance token, so modifying the
            # keypoint slice in-place invalidates autograd's saved version.
            instance_tokens = tensor[:, :, :-np]
            keypoint_tokens = tensor[:, :, -np:] + pos
            tensor = torch.cat((instance_tokens, keypoint_tokens), dim=2)
        return tensor
...
class DETRTransformer(nn.Module):
    ...
    def __init__(
        ...
        nn.init.normal_(self.keypoint_embedding.weight, mean=0.0, std=0.01)
        ...
        nn.init.normal_(self.pose_enc.weight, mean=0.0, std=0.01)
    ...
    def forward(self, feats, targets, samples=None):
        ...
        if self.training and targets is not None:
            input_query_label, input_query_pose, attn_mask, dn_meta = prepare_for_cdn(
                ...
                keypoint_oks_sigmas=self.keypoint_oks_sigmas
                )
            # A batch made entirely of background images has no ground-truth
            # instances from which to build denoising queries. In that case,
            # prepare_for_cdn returns None and the regular matching queries are
            # sufficient for learning the background classification loss.
            if input_query_label is not None:
                ...
                # Keep the DN embeddings in DDP's graph for an all-background
                # micro-batch. The dependency is exactly zero, so predictions
                # and losses are unchanged, while both parameters receive a
                # valid zero gradient and DDP can use its fast reduction path.
                dn_zero_grad = (self.label_enc.weight.sum() + self.pose_enc.weight.sum()) * 0.0
                tgt_pose = tgt_pose + dn_zero_grad
        else:
        ...
        # The pre-decoder poses are returned as (..., keypoints, 2) during
        # training and must be flattened whether or not DN queries exist.
        if self.training:
            out_pre_poses = out_pre_poses.flatten(-2)
            out_refs = out_references
        ...
        # Clamp outputs to prevent fp16 overflow
        for k in out:
            if isinstance(out[k], torch.Tensor):
                out[k] = torch.nan_to_num(out[k], nan=0.0, posinf=1e4, neginf=-1e4)
                out[k] = out[k].clamp(-1e4, 1e4)
