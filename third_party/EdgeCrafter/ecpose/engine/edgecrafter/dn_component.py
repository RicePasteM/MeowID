...
def get_sigmas(num_keypoints, device, keypoint_oks_sigmas=None):
    if keypoint_oks_sigmas is not None:
        if len(keypoint_oks_sigmas) != num_keypoints:
            raise ValueError(
                f'Expected {num_keypoints} keypoint sigmas, '
...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, num_keypoints,
                    hidden_dim, label_enc, pose_enc, img_dim, device,
    ...
    if training:
        ...
        vars = (2 * get_sigmas(
            num_keypoints, device, keypoint_oks_sigmas
        )) ** 2
