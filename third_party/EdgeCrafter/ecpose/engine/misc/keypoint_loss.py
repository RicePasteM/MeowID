    ...
    def __init__(self,
        ...
        if sigmas is not None:
            if len(sigmas) != num_keypoints:
                raise ValueError(
                    f'Expected {num_keypoints} keypoint sigmas, got {len(sigmas)}'
                )
            ...
            # Cat face keypoints: left_eye, right_eye, mouth, left_ear1-3, right_ear1-3
            self.sigmas = np.array([
                .30, .30,  # left_eye, right_eye
                .35,       # mouth
                .50, .50, .50,  # left_ear1-3
                .50, .50, .50   # right_ear1-3
            ], dtype=np.float32) / 10.0
